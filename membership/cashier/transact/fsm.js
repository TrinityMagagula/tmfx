...
const supabase = getSupabaseClient();
        ...
        // Function to get FSM Yearly price from ItemList table
        async function getMentorshipPrice() {
            try {
                ...
                if (data && data['FSM']) {
                    mentorshipPrice = data['FSM'];
                    document.getElementById('subscriptionText7').textContent = 
                        `Subscribe to Forex Signals Monthly For R${mentorshipPrice}`;
                }
            } catch (error) {
                console.error('Error fetching Forex Signals Monthly price:', error);
            }
        }
        ...
        async function FSM() {
            ...
            try {
                ...
                const popup = document.getElementById('mentorshipPopup7');
        ...
        function showAlreadySubscribedPopup() {
            const popup = document.getElementById('alreadySubscribedPopup7');
            popup.classList.add('active');
        }
        ...
        async function handleSubscription() {
            // Get elements
            const confirmBtn = document.getElementById('confirmBtn7');
            const loadingSpinner = document.getElementById('loadingSpinner7');
            const subscriptionText = document.getElementById('subscriptionText7');
            const doneMessage = document.getElementById('doneMessage7');
            ...
            try {
                ...
                const { data: transactionData, error: transactionError } = await supabase
                    ...
                    .insert([{
                        id: currentUser.uid,
                        ItemName: 'Forex Signals Monthly',
                        Date: date,
                        Quantity: 1,
                        Amount: - amount,
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('confirmBtn7').addEventListener('click', handleSubscription);
            
            // Close button click (subscription popup)
            document.getElementById('closeBtn7').addEventListener('click', () => hidePopup('mentorshipPopup7'));
            
            // Close button click (already subscribed popup)
            document.getElementById('alertCloseBtn7').addEventListener('click', () => hidePopup('alreadySubscribedPopup7'));
        });

        // Make FSM function available globally
        window.FSM = FSM;