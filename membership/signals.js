  ...
  async function checkUserSubscription(uid) {
    try {
      ...
      const [DSM, DSW, FSM, FSW] = await Promise.all([
        supabase.from('DerivSignalsMonthly').select('id').eq('id', uid),
        supabase.from('DerivSignalsWeekly').select('id').eq('id', uid),
        supabase.from('FSM').select('id').eq('id', uid),
        supabase.from('FSW').select('id').eq('id', uid)
      ]);

      // Check if user exists in any of the tables
      const hasDSM = DSM.data && DSM.data.length > 0;
      const hasDSW = DSW.data && DSW.data.length > 0;
      const hasFSM = FSM.data && FSM.data.length > 0;
      const hasFSW = FSW.data && FSW.data.length > 0;

      return hasDSM || hasDSW || hasFSW || hasFSM;
    } catch (error) {
  ...
  async function goToSignalsPage() {
    ...
    if (isSubscribed) {
      ...
      alert('Please subscribe to access signals');
