...
async function checkUserAccess(uid) {
    try {
        ...
        const [monthlySignals, weeklySignals, fsw, fsm] = await Promise.all([
        ...
        const hasAccess = 
            (monthlySignals.data && monthlySignals.data.length > 0) ||
            (weeklySignals.data && weeklySignals.data.length > 0) ||
            (fsw.data && fsw.data.length > 0) ||
            (fsm.data && fsm.data.length > 0);
