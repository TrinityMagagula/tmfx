...
const updateAccountMetrics = async () => {
  const { data: past } = await supabase.from('past-tradespk').select('profit').eq('id', uid);
  if (past) balance = round2(past.reduce((sum, row) => sum + Number(row.profit || 0), 0));

  const { data: open } = await supabase.from('open-tradespk').select('volume').eq('id', uid);
...
const loadOpenTrades = async () => {
  const { data: trades } = await supabase.from('open-tradespk').select('*').eq('id', uid);
  if (!trades) return;
...
window.closeTrade = async (tradeid) => {
  ...
  await supabase.from('past-tradespk').insert([{
    tradeid: tid,
    symbol,
    time,
    entry: round2(entry),
  ...
  await supabase.from('open-tradespk').delete().eq('tradeid', tradeid);
