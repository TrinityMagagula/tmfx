  ...
  async function checkUserSubscription(uid) {
    try {
      ...
      const hasMonthly = monthlyResult.data && monthlyResult.data.length > 0;
      const hasWeekly = weeklyResult.data && weeklyResult.data.length > 0;
      const hasYearly = yearlyResult.data && yearlyResult.data.length > 0;

      return hasMonthly || hasWeekly || hasYearly;
    } catch (error) {
  ...
  async function goToMentorshipPage() {
    ...
    if (isSubscribed) {
      ...
      alert('Please subscribe to access mentorship');
