...
// Max age in seconds for each timeframe
const retentionLimits = {
  '1m': 30 * 24 * 60 * 60,     // 30 days
  '5m': 60 * 24 * 60 * 60,     // 60 days
  '15m': 90 * 24 * 60 * 60,    // 90 days
  '30m': 120 * 24 * 60 * 60,   // 120 days
  '1h': 180 * 24 * 60 * 60,    // 180 days
  '4h': 240 * 24 * 60 * 60,    // 240 days
  '1d': 365 * 24 * 60 * 60,    // 365 days
  '1w': 365 * 24 * 60 * 60,
  '1mn': 365 * 24 * 60 * 60,
};
...
function cleanOldCandles(tf, ref) {
  const maxAge = retentionLimits[tf];
  const cutoffTime = Math.floor(Date.now() / 1000) - maxAge;

  ref.once('value', snapshot => {
    snapshot.forEach(child => {
      const candle = child.val();
      if (candle.time < cutoffTime) {
        child.ref.remove().catch(console.error);
      }
    });
  });
}
...
function updateCandles(price, timestamp) {
  for (const [tf, tracker] of Object.entries(candleTrackers)) {
    ...
    if (elapsed >= timeframes[tf]) {
      ...
      tracker.ref.push(finalizedCandle).then(() => {
        cleanOldCandles(tf, tracker.ref); // clean old candles after pushing new one
      });
