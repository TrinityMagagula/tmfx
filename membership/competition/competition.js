  ...
  async function checkUserSubscription(uid) {
    try {
      ...
      const hasMCS = DMC.data && DMC.data.length > 0;
      const hasDMC = MCS.data && MCS.data.length > 0;

      return hasDMC || hasMCS;
    } catch (error) {
  ...
  async function goToCompetitionPage() {
    ...
    if (isSubscribed) {
      window.location.href = '../competition/dashboard/dashboard.html';
    } else {
      alert('Please subscribe to access competition');
      window.location.href = '../cashier/transact/subscriptions.html';
    }
  }

  // Make the function available globally
  window.goToCompetitionPage = goToCompetitionPage;
