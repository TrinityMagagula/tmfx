...
const supabase = getSupabaseClient();
        ...
        // Function to get MCS Yearly price from ItemList table
        async function getMentorshipPrice() {
            try {
                ...
                if (data && data['MCS']) {
                    mentorshipPrice = data['MCS'];
                    document.getElementById('subscriptionText8').textContent = 
                        `Subscribe to Special For R${mentorshipPrice}`;
                }
            } catch (error) {
        ...
        async function MCS() {
            ...
            try {
                ...
                const popup = document.getElementById('mentorshipPopup8');
        ...
        function showAlreadySubscribedPopup() {
            const popup = document.getElementById('alreadySubscribedPopup8');
            popup.classList.add('active');
        }
        ...
        async function handleSubscription() {
            // Get elements
            const confirmBtn = document.getElementById('confirmBtn8');
            const loadingSpinner = document.getElementById('loadingSpinner8');
            const subscriptionText = document.getElementById('subscriptionText8');
            const doneMessage = document.getElementById('doneMessage8');
            ...
            try {
                ...
                const { data: transactionData, error: transactionError } = await supabase
                    ...
                    .insert([{
                        id: currentUser.uid,
                        ItemName: 'Mentorship + Competitioin = Special',
                        Date: date,
                        Quantity: 1,
                        Amount: - amount,
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('confirmBtn8').addEventListener('click', handleSubscription);
            
            // Close button click (subscription popup)
            document.getElementById('closeBtn8').addEventListener('click', () => hidePopup('mentorshipPopup8'));
            
            // Close button click (already subscribed popup)
            document.getElementById('alertCloseBtn8').addEventListener('click', () => hidePopup('alreadySubscribedPopup8'));
        });

        // Make MCS function available globally
        window.MCS = MCS;