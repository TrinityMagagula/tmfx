  ...
  async function checkUserSubscription(uid) {
    try {
      ...
      const [ nk] = await Promise.all([

        supabase.from('nk').select('id').eq('id', uid),

      ]);

      // Check if user exists in any of the tables

      const haspk = nk.data && nk.data.length > 0;
  ...
  async function goToNationalKO() {
    ...
    if (isSubscribed) {
      window.location.href = 'nationalknockouts/natko.html';
    } else {
  ...
  window.goToNationalKO = goToNationalKO;
