  ...
  async function checkUserSubscription(uid) {
    try {
      ...
      const [ pk] = await Promise.all([
        ...
        supabase.from('pk').select('id').eq('id', uid),

      ]);

      // Check if user exists in any of the tables

      const haspk = pk.data && pk.data.length > 0;
  ...
  async function goToProvincialKO() {
    ...
    if (isSubscribed) {
      window.location.href = 'provincialknockouts/provko.html';
    } else {
  ...
  window.goToProvincialKO = goToProvincialKO;
