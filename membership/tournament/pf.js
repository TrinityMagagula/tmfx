  ...
  async function checkUserSubscription(uid) {
    try {
      ...
      const [ pf] = await Promise.all([
        ...
        supabase.from('pf').select('id').eq('id', uid),

      ]);

      // Check if user exists in any of the tables

      const haspk = pf.data && pf.data.length > 0;
  ...
  async function goToProvincialFinals() {
    ...
    if (isSubscribed) {
      window.location.href = 'provincialfinals/provfinal.html';
    } else {
  ...
  window.goToProvincialFinals = goToProvincialFinals;
