...
const supabase = getSupabaseClient();
        ...
        // Function to get Mentorship Monthly price from ItemList table
        async function getMentorshipPrice() {
            try {
                const { data, error } = await supabase
                    .from('ItemList')
                    .select('"Mentorship Monthly"')
                    .single();

                if (error) throw error;
                if (data && data['Mentorship Monthly']) {
                    mentorshipPrice = data['Mentorship Monthly'];
                    document.getElementById('subscriptionText2').textContent = 
                        `Subscribe to Monthly Mentorship For R${mentorshipPrice}`;
                }
            } catch (error) {
        ...
        async function MM() {
            ...
            try {
                ...
                const popup = document.getElementById('mentorshipPopup2');
        ...
        function showAlreadySubscribedPopup() {
            const popup = document.getElementById('alreadySubscribedPopup2');
            popup.classList.add('active');
        }
        ...
        async function handleSubscription() {
            ...
            const subscriptionText = document.getElementById('subscriptionText2');
            const doneMessage = document.getElementById('doneMessage2');
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('confirmBtn2').addEventListener('click', handleSubscription);
            
            // Close button click (subscription popup)
            document.getElementById('closeBtn2').addEventListener('click', () => hidePopup('mentorshipPopup2'));
            
            // Close button click (already subscribed popup)
            document.getElementById('alertCloseBtn2').addEventListener('click', () => hidePopup('alreadySubscribedPopup2'));
        });

        // Make MM function available globally
        window.MM = MM;