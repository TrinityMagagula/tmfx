...
const updateAccountMetrics = async () => {
  const { data: past } = await supabase.from('past-trades').select('profit').eq('id', uid);
  if (past) balance = round2(past.reduce((sum, row) => sum + Number(row.profit || 0), 0));

  const { data: open } = await supabase.from('open-trades').select('volume').eq('id', uid);
...
const loadOpenTrades = async () => {
  const { data: trades } = await supabase.from('open-trades').select('*').eq('id', uid);
  if (!trades) return;
...
window.closeTrade = async (tradeid) => {
  ...
  await supabase.from('past-trades').insert([{
    tradeid: tid,
    symbol,
    time,
    entry: round2(entry),
  ...
  await supabase.from('open-trades').delete().eq('tradeid', tradeid);
