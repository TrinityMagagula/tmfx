...
const supabase = getSupabaseClient();
        ...
        async function getMentorshipPrice() {
            try {
                ...
                if (data && data['FMS']) {
                    mentorshipPrice = data['FMS'];
                    document.getElementById('subscriptionTextz').textContent = 
                        `Book F2F Session For R${mentorshipPrice}`;
                }
            } catch (error) {
                console.error('Error fetching F2F Session price:', error);
            }
        }
        ...
        async function FMS() {
            ...
            try {
                ...
                const popup = document.getElementById('mentorshipPopupz');
        ...
        function showAlreadySubscribedPopup() {
            const popup = document.getElementById('alreadySubscribedPopupz');
            popup.classList.add('active');
        }
        ...
        async function handleSubscription() {
            // Get elements
            const confirmBtn = document.getElementById('confirmBtnz');
            const loadingSpinner = document.getElementById('loadingSpinnerz');
            const subscriptionText = document.getElementById('subscriptionTextz');
            const doneMessage = document.getElementById('doneMessagez');
            const closeBtn = document.getElementById('closeBtnz');
            ...
            try {
                ...
                const { data: transactionData, error: transactionError } = await supabase
                    ...
                    .insert([{
                        id: currentUser.uid,
                        ItemName: 'F2F Mentorship Session',
                        Date: date,
                        Quantity: 1,
                        Amount: - amount,
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('confirmBtnz').addEventListener('click', handleSubscription);
            
            // Close button click (subscription popup)
            document.getElementById('closeBtnz').addEventListener('click', () => hidePopup('mentorshipPopupz'));
            
            // Close button click (already subscribed popup)
            document.getElementById('alertCloseBtnz').addEventListener('click', () => hidePopup('alreadySubscribedPopupz'));
        });
