...
const supabase = getSupabaseClient();
        ...
        // Function to get OMS Yearly price from ItemList table
        async function getMentorshipPrice() {
            try {
                ...
                if (data && data['OMS']) {
                    mentorshipPrice = data['OMS'];
                    document.getElementById('subscriptionTextd').textContent = 
                        `Book Online Session For R${mentorshipPrice}`;
                }
            } catch (error) {
                console.error('Error fetching Online Session price:', error);
            }
        }
        ...
        async function OMS() {
            ...
            try {
                ...
                const popup = document.getElementById('mentorshipPopupd');
        ...
        function showAlreadySubscribedPopup() {
            const popup = document.getElementById('alreadySubscribedPopupd');
            popup.classList.add('active');
        }
        ...
        async function handleSubscription() {
            // Get elements
            const confirmBtn = document.getElementById('confirmBtnd');
            const loadingSpinner = document.getElementById('loadingSpinnerd');
            const subscriptionText = document.getElementById('subscriptionTextd');
            const doneMessage = document.getElementById('doneMessaged');
            const closeBtn = document.getElementById('closeBtnd');
            ...
            try {
                ...
                const { data: transactionData, error: transactionError } = await supabase
                    ...
                    .insert([{
                        id: currentUser.uid,
                        ItemName: 'Online Mentorship Session',
                        Date: date,
                        Quantity: 1,
                        Amount: - amount,
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('confirmBtnd').addEventListener('click', handleSubscription);
            
            // Close button click (subscription popup)
            document.getElementById('closeBtnd').addEventListener('click', () => hidePopup('mentorshipPopupd'));
            
            // Close button click (already subscribed popup)
            document.getElementById('alertCloseBtnd').addEventListener('click', () => hidePopup('alreadySubscribedPopupd'));
        });

        // Make DMC function available globally
        window.OMS = OMS;