...
const supabase = getSupabaseClient();
        ...
        // Function to get Mentorship Yearly price from ItemList table
        async function getMentorshipPrice() {
            try {
                const { data, error } = await supabase
                    .from('ItemList')
                    .select('"Mentorship Yearly"')
                    .single();

                if (error) throw error;
                if (data && data['Mentorship Yearly']) {
                    mentorshipPrice = data['Mentorship Yearly'];
                    document.getElementById('subscriptionText3').textContent = 
                        `Subscribe to Yearly Mentorship For R${mentorshipPrice}`;
                }
            } catch (error) {
        ...
        async function MY() {
            ...
            try {
                ...
                const popup = document.getElementById('mentorshipPopup3');
        ...
        function showAlreadySubscribedPopup() {
            const popup = document.getElementById('alreadySubscribedPopup3');
            popup.classList.add('active');
        }
        ...
        async function handleSubscription() {
            ...
            const subscriptionText = document.getElementById('subscriptionText3');
            const doneMessage = document.getElementById('doneMessage3');
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('confirmBtn3').addEventListener('click', handleSubscription);
            
            // Close button click (subscription popup)
            document.getElementById('closeBtn3').addEventListener('click', () => hidePopup('mentorshipPopup3'));
            
            // Close button click (already subscribed popup)
            document.getElementById('alertCloseBtn3').addEventListener('click', () => hidePopup('alreadySubscribedPopup3'));
        });
